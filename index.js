...
// Create a separate axios instance for OpenAI requests
...
//   });
  ...
  // Initialize OpenAI with the custom axios instance
//   const openai = new OpenAI({
//     apiKey: process.env.OPENAI_API_KEY,
//     axios: openaiAxios
//   });
    ...
    try {
        ...
        app.get('/test-openai', async (req, res) => {
            try {
                ...
                res.status(500).json({ error: 'Failed to connect to OpenAI API', details: error.message });
            }
            });
        //Accept a prompt from the user and return recommended courses
        app.post('/recommendations',  async (req, res) => {
            ...
            const availableCourses = await classesCollection.find({ status: 'approved' }).toArray();
            
            // Create a string of course titles
            const courseTitles = availableCourses.map(course => course.name).join(', ');
        
            // Create the GPT-3 prompt
            const gptPrompt = `Given the following list of available courses: ${courseTitles}
            
            And based on the user's input: "${userPrompt}"
            
            Recommend up to 5 courses from the available list that would be most relevant. 
            Provide a brief explanation for each recommendation.
            
            Format the response as a JSON object with "recommendations" as the key and an array of objects as the value. 
            Each object should have "course" and "explanation" as keys.`;
